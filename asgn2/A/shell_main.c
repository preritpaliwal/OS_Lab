...
cmd** tokenise_on_pipe(char * user_input, char *err, int *err_flag, int *num_cmds){
    ...
    for (int i = 0; i < strlen(user_input) - 1; i++){
        if (user_input[i] == '"'){
            ...
            if (i == strlen(user_input) - 2){  // the user has entered a command like "ls | wc |"
            ...
            if (strlen(temp) != 0){
                ...
                while(user_input[i+1] == ' '  && i < strlen(user_input) - 1 ){ // remove the unnnecesarry spaces after pipe
                    i++;
                }
                if (i == strlen(user_input)){  // the user has entered a command like "ls | wc |   "
                    strcpy(err, "Invalid command! No command after a pipe.\n");
                    *err_flag = -1;
                    return NULL;
                }
            ...
            else{  // the user has entered a command like "ls || wc" or the cmd starts with a pipe
