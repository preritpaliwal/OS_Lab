...
static void manage_child(int sig)
{
    ...
    while (true)
    {
        ...
        if (ret_pid == 0)
        { // there are one or more child processes in execution, but none of them have exited
            break;
        }

        else if (ret_pid == -1)
        { // there are no child processes in execution, or there was an error
            // perror("waitpid");
            break;
        }
        ...
        {
            if (ret_pid == fgpid)
            {              // if the child process that exited was the foreground process
                fgpid = 0; // set the fgpid to 0 to denote that no foreground process is running from now on
            }

            if (fg_processes_pid.find(ret_pid) != fg_processes_pid.end())
            {                                    // if the process that exited was a foreground process
            ...
            if (bg_processes_pid.find(ret_pid) != bg_processes_pid.end())
            {                                    // if the process that exited was a background process
...
void handle_process(cmd **cmd_seq, int *num_piped_cmds, int background)
{
    ...
    for (int i = 0; i < *num_piped_cmds; i++)
    {
        ...
        if (i > 0)
        { // for all other commands except 1st one in pipe , set the input file descriptor to the read end of the pipe
        ...
        if (i == *num_piped_cmds - 1)
        {                             // if the command is the last command in the pipe
        ...
        if (child_pid == 0)
        {
            ...
            if (cmd_seq[i]->in_fd != STDIN_FILENO)
            { // if the input file descriptor of the command is not STDIN_FILENO, then close the STDIN_FILENO file descriptor
                // close(STDIN_FILENO);
            ...
            if (cmd_seq[i]->out_fd != STDOUT_FILENO)
            { // if the output file descriptor of the command is not STDOUT_FILENO, then close the STDOUT_FILENO file descriptor
                // close(STDOUT_FILENO);
        ...
        if (curr_pg_id == 0)
        {
            ...
            if (background == 0)
            {                                        // for the process grp with curr_pg_id (this process is assumed to start in fg), set it as the foreground process grp on the terminal associated with the stdin file descriptor
        ...
        if (i < *num_piped_cmds - 1)
        { // close the read end of the pipe, if it is not the last command in the pipe
            close(cmd_seq[i]->out_fd);
        }
    }
    ...
    { // if the command is not a background process, wait for the last command in the pipe to finish executing
