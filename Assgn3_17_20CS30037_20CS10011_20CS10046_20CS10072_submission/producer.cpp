...
int calc_degree(int *shm1, int *shm2, int num_nodes, int num_edges, vector <pair<int, int>> &node_degree, vector <pair<int, double>> &node_prob){
    ...
    cout << "Degree of each node calculated.\n";  

    // sort the nodes in decreasing order of their degree
    // sort(node_degree.begin(), node_degree.end(), [](pair<int, int> a, pair<int, int> b){
    //     return a.second > b.second;
    // });
    sort(node_degree.begin(), node_degree.end(), sort_by_sec_desc);
...
int main(){
    ...
    while(1){
        ...
        for (int i = 0; i < num_new_nodes; i++){
            ...
            // sample a random number in [1, 20] uniformly => k
            // this new node connects to k existing nodes in the graph

            cout << "New node: " << num_nodes + i << endl;

            int k = 1 + rand() % 8;
            cout << "Number of edges: " << k << endl;
            ...
            for (int j = 0; j < k; j++){
                ...
                cout << "Edge: " << num_nodes + i << " " << node_degree[idx].first << endl;
            }
            cout << endl << endl;      
        }
        ...
        for (int i = 0; i < new_edges.size(); i++){
            ...
            // cout << "Idx: " << 2 + num_edges + i << ", " << new_edges[i].first << endl;
